...
import cnam.nsy209.selServices.association.client.view.page.concretePage.main.FastMessage;
...
public class FastMessagePage extends AbstractPage implements Observer {

	/* Constructor */
	public FastMessagePage(int width, int height) {
		super(width, height);
	}
	...
	public IDisplayStrategy getDisplayStrategy(int width, int height, String title, LocalDto dto) {
		...
		displayStrategy.addPanel(UpTitle.get(EnumMenuHorizontal.FAST_MESSAGE));
		displayStrategy.addPanel(Menu.get());
		displayStrategy.addPanel(FastMessage.get());
