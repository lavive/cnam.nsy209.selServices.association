...
import cnam.nsy209.selServices.association.client.view.page.concretePage.upBand.MenuDemands;
...
public class DemandsDisplayPage extends AbstractPage implements Observer {

	/* Constructor */
	public DemandsDisplayPage(int width, int height) {
		super(width, height);
	}
	...
	public IDisplayStrategy getDisplayStrategy(int width, int height, String title, LocalDto dto) {
		...
		displayStrategy.addPanel(MenuDemands.get());
		displayStrategy.addPanel(Menu.get());
		displayStrategy.addPanel(DemandsDisplay.get());
